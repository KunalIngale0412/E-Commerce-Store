...
export async function POST(request) {
  try {
    ...
    // get user data
    const getUser = await UserModel.findOne({ deletedAt: null , email }).select("+password +isEmailVerfied")
    if(!getUser){
        return response(false, 400, "Invalid login credentials.")
    }
    console.log('User ID:', getUser._id.toString(), 'Current isEmailVerified status:', getUser.isEmailVerfied);
// 🛑 END DEBUG LINE 🛑
    // resend email verification link
    if(!getUser.isEmailVerfied){
...
// password verification
    const isPasswordVerfied = await getUser.comparePassword(password)
